...
private:
    ...
    node *perform_avl_tree_process(node *temp){
        ...
        if(balance>1 && balance_factor(temp->left)>=0){
            // LL case for rotation
            return right_rotation(temp);
        }else if(balance>1 && balance_factor(temp->left)<0){
            // LR case for rotation
            temp->left = left_rotation(temp->left);
            return right_rotation(temp);
        }else if(balance<-1 && balance_factor(temp->right)<=0){
            // RR case for rotation
            return left_rotation(temp);
        }else if(balance<-1 && balance_factor(temp->right)>0){
            // RL case for rotation
    ...
    node *insert_node(node *temp, int item){
        ...
        temp = perform_avl_tree_process(temp);
        return temp;
    }
